		       ...
		       String alreadyDone = firstFormatter.format(firstNow);
				 ...
				 //check if today's progress has been recorded or not
	              if ((listOfStudents.get(index).getLastRecord().equals(alreadyDone))) {
			       ...
			       //IF NOT WORKING TRY ADDING HORIZNTAL FIELD TO VERTICAL FIELD AND ADDING VERTICAL LAYOIT TO ADD EVERY TIME
			       ...
			       if ((listOfStudents.get(index).getProgramChosen()).equals("hafiz")) {
			    	   ...
			    	   H1 dourTitle = new H1("Dour");
			      	      ...
			      	      dour.add(dourTitle);
			            Paragraph info = new Paragraph ("Dour: " + listOfStudents.get(index).getFullName()+" is on quarter number " + listOfStudents.get(index).getCurrentQuarter() + " of sapara " + listOfStudents.get(index).getDourCurrentSapara() + ".");
			       ...
			       //if the program chosen is not hafix
	            	  ...
	            	  VerticalLayout dour = new VerticalLayout(); 
			    	   H1 dourTitle = new H1("Dour");
			      	      dour.add(dourTitle);
			            Paragraph info = new Paragraph ("Dour Progress: " + listOfStudents.get(index).getFullName()+" is on quarter number " + listOfStudents.get(index).getCurrentQuarter() + " of sapara " + listOfStudents.get(index).getDourCurrentSapara() + ".");
			            ...
			            VerticalLayout sapara = new VerticalLayout(); 
				    	   H1 saparaTitle = new H1("Sabaq");
				            ...
				            Paragraph info3 = new Paragraph ("Sabaq Progress: " + listOfStudents.get(index).getFirstName()+" is memorizing sapara " + listOfStudents.get(index).getCurrentSaparaMemorizing());
				        ...
				        sorted = sorted + arrayOfToSort [k];
				            ...
				            Button complete1 = new Button ("Yes", f -> {
						        ...
						        Button memorized = new Button ("Yes", c -> {
					                ...
					                IntegerField newSaparaa = new IntegerField("Enter the new sapara that they are starting");
					                ...
					                Button donee = new Button("Done", k -> {
					                	 ...
					                	 int newSapara = newSaparaa.getValue();
							                ...
							                if(listOfStudents.get(index).getTotalSaparasDone() >= 30) {
							                  listOfStudents.get(index).setProgramChosen("hafiz");
							                  Notification.show("This student is now a hafiz. There program has now sucessfully changed. To change it back, go to 'Change type of student option.'",
						        ...
						        Button notMemorized = new Button("No", l -> {
						        	listOfStudents.get(index).setTodayLinesMemorized(numLines.getValue());
					            	listOfStudents.get(index).setTodayMistakesMade(numMistakes.getValue());
