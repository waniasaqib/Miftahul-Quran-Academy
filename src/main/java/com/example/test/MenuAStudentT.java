...
@Route(value = "menuAStudentT", layout = Welcome.class)
public class MenuAStudentT extends VerticalLayout {
	...
	public MenuAStudentT() {
	    ...
	    select.setItems("Present", "Absent");
		 ...
		 textField.setPlaceholder(listOfStudents.get(index).getReasonAbsent().get(listOfStudents.get(index).getReasonAbsent().size()-1));
		 
		 if ((listOfStudents.get(index).getAttendance().get(listOfStudents.get(index).getAttendance().size()-1)) == true) {
		 ...
		 textField2.setPlaceholder(listOfStudents.get(index).getReasonCovidScreening().get(listOfStudents.get(index).getReasonCovidScreening().size()-1));
		 
		 select2.setLabel("COVID Screening");
		    select2.setItems("Complete", "Incomplete");
		    if ((listOfStudents.get(index).getCovidScreening().get(listOfStudents.get(index).getCovidScreening().size()-1)) == true) {
		 ...
		 HorizontalLayout edit = new HorizontalLayout(select, textField, select2, textField2);
		    ...
		    Button done = new Button("Done", e -> {
		    	 int ind = -1;
		          ...
		          listOfStudents.get(index).getReasonAbsent().set(ind, input);
		          boolean set2;
		          String input1;
		          if (select2.getValue() == "Present") {
		        	  ...
		        	  set2 = false;
		        	  input1 = textField2.getValue();
		          }
		          listOfStudents.get(index).getCovidScreening().set(ind, set2);
		          listOfStudents.get(index).getReasonCovidScreening().set(ind, input1);
		        } 
		      }
